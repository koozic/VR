...
export function createArtworkFrame(artwork) {
  ...
  if (artwork.imageUrl) {
    loadGifTexture(artwork.imageUrl, canvasMat, gifState, canvasMesh, maxW, maxH);
  } else {
    const seed = artwork.id != null ? Number(artwork.id) || 1 : 1;
    const artCanvas = generateArtCanvas(artwork.title, seed);
