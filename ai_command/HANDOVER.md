...
## 2026-06-15 원격 유저 모델 방향 및 걷기 모션 수정

- `frontend/src/three/createRemoteVisitor.js`에서 같은 이동 상태가 매 프레임 전달될 때 애니메이션 전환을 반복 시작하던 문제를 수정했다.
- 현재 이동 상태를 저장하고 걷기/대기 상태가 실제로 변경될 때만 크로스페이드하며 전환 시간은 150ms다.
- `frontend/src/three/syncRemoteVisitors.js`는 서버 원본 좌표 변화로 이동을 판단하고 마지막 이동 패킷 후 120ms 동안 걷기 상태를 유지한다.
- `frontend/src/three/loadBlockyCharacter.js`는 모델 로컬 Y축을 `Math.PI`만큼 회전해 네트워크 yaw와 정면 방향을 맞춘다.
- `npm run build`와 `git diff --check`는 통과했다.
- TASK-027 전체 수동 검증은 미완료이며, 다중 브라우저에서 정면 방향, 이동 중 걷기 지속, 정지 후 대기 모션 복귀를 확인해야 한다.
...
### 2026-06-16 추가 작업: Space Gallery 전시 데이터 갱신 및 시야 기반 UI 선택
...
| 작업 ID | TASK-037 |
| 상태 | DONE, 커밋 예정 |
| 완료 | `Cosmos Odyssey`와 `Stellar Cradle` 전시 데이터, 이미지 URL, 설명문, 대화 프로필을 갱신했다. `shared/gallery-seed.json` version을 5로 올렸다. 카메라 forward 벡터를 이용해 UI 포커스와 HUD 근접 전시물 선택이 시야 방향을 반영하게 했다. |
| 먼저 확인할 파일 | `shared/gallery-seed.json`, `frontend/src/three/distanceCheck.js`, `frontend/src/three/GalleryScene.jsx`, `frontend/src/three/createExhibitFrame.js`, `frontend/src/curator/exhibitConversationProfiles.js` |
| 주의 사항 | 기존 ai_command 문서 일부는 인코딩이 깨져 있어 새 기록은 파일 끝에 UTF-8로 추가했다. seed 변경은 DB 재시드가 필요하므로 backend 실행 환경에서 version 5 적용 여부를 확인한다. |
| 검증 결과 | `npm run build` 성공. `node --check frontend/src/three/createExhibitFrame.js`, `node --check frontend/src/three/distanceCheck.js` 성공. |
| 남은 작업 | 실제 브라우저에서 Space Gallery 진입 후 GIF 애니메이션, 발광 효과, 카메라 시야 기반 HUD/정보 패널 전환을 수동 확인한다. |

### 2026-06-18 추가 작업: WebLLM 응답속도 개선 반영 확인
...
| 작업 ID | TASK-038 |
| 상태 | DONE |
| 완료 | pull된 현재 커밋(`4622bf4`)에서 WebLLM 사전 준비, 스트리밍 응답 표시, 최근 대화 6개 제한, `max_tokens: 160`, 모델 준비 상태 UI가 반영됐음을 확인했다. |
| 먼저 확인할 파일 | `frontend/src/api/webLlmApi.js`, `frontend/src/pages/GalleryPage.jsx`, `frontend/src/components/DocentSpeechBubble.jsx`, `frontend/src/styles.css`, `ai_command/WORK_LOG.md` |
| 검증 결과 | `npm run build` 성공. Playwright + Chrome WebGPU 환경에서 이전 커밋(`7f1418e`)과 비교했다. 이전 cold 요청은 약 46.24~48.22초였고 중간 표시가 없었다. 현재 버전은 warm 기준 첫 토큰 6.39~8.97초, 최종 생성 14.67~19.48초를 확인했다. 실제 앱 UI에서는 약 27.51초 후 `AI 모델 준비 완료`가 표시됐다. |
| 판단 | 순수 warm 생성 시간은 큰 폭으로 줄었다고 보기는 어렵지만, 사전 로딩과 토큰 스트리밍 덕분에 사용자가 느끼는 첫 응답 대기는 명확히 개선됐다. |
| 주의 사항 | 최초 방문자는 여전히 모델 fetch/cache와 WebGPU 초기화가 필요하다. 스트리밍 중간 텍스트는 최종 검증 전 화면에 표시될 수 있으므로 사실 검증 정책을 후속 점검한다. 백엔드 미구동 상태에서는 `/api`, `/ws` 콘솔 오류가 날 수 있으나 WebLLM 자체 검증과는 별개다. |
## 2026-06-22: 현재 프로젝트 구조 보고서

| 항목 | 내용 |
| --- | --- |
| 완료된 부분 | `ai_command/PROJECT_STRUCTURE_REPORT.md`를 새로 작성했다. |
| 포함 내용 | 전체 디렉터리 개요, 프론트엔드/백엔드/AI 서버 구조, shared seed, docs/ai_command 문서 체계, 실행/검증 명령, 산출물 주의사항, 구조상 리스크와 미처리 확인점. |
| 확인 기준 | `README.md`, `frontend/package.json`, `backend/pom.xml`, `ai-server/requirements.txt`, 실제 디렉터리 목록과 주요 소스 파일. |
| 다음 작업 | 구조 변경, 신규 서비스 추가, 실행 스크립트 변경 시 보고서를 함께 갱신한다. |

## 2026-06-22: TASK-029 커밋 전 정리

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료된 부분 | room geometry 재사용, trim/grid `InstancedMesh`, 공통 `assetLoader.js`, 우주관/역사관/도슨트/원격 방문자 GLB 로더 캐시 전환, asset/runtime/scene transition/remote visitor 측정 스크립트 추가. |
| 검증 | `npm run build` 성공. `node --check` 성공. Playwright 기반 asset/runtime/scene transition/remote visitor 측정 성공. warm 구간 asset resource timing 0 확인. |
| 미처리 작업 | 실제 키보드/포인터락 포탈 충돌 E2E 측정, FPS/frame time/heap/GPU memory 계측, asset cache eviction/lifetime 정책, 측정 스크립트 별도 문서화, Edge/모바일/저사양 환경 검증. |
| 커밋 메모 | `ai_command/`는 `.gitignore` 대상이므로 이번 커밋에 포함하려면 `git add -f ai_command/TASK_BOARD.md ai_command/WORK_LOG.md ai_command/HANDOVER.md`가 필요하다. |
| 다음 작업 | TASK-029 잔여 항목은 별도 QA/성능 계측 작업으로 분리한다. |

## 2026-06-22: TASK-029 원격 방문자 캐릭터 cache 측정

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료된 부분 | `frontend/scripts/measure-remote-visitors.mjs`, `frontend/src/three/remoteVisitorMeasureHarness.js`, `npm run measure:remote-visitors`를 추가했다. 원격 방문자 18명 생성/제거 시 캐릭터 GLB 캐시와 animation mixer 생성 상태를 측정한다. |
| 실행 방법 | 프론트 dev server 실행 후 `npm.cmd run measure:remote-visitors -- --chrome="C:/Program Files/Google/Chrome/Application/chrome.exe" --count=18` |
| 검증 | `node --check scripts/measure-remote-visitors.mjs` 성공, `node --check src/three/remoteVisitorMeasureHarness.js` 성공, `npm run build` 성공, 측정 스크립트 실행 성공. |
| 측정 결과 | primary ids cold 0.14s / resources 10, warm 0.06s / resources 0. secondary ids cold 0.07s / resources 8, warm 0.06s / resources 0. 총 18종 캐릭터 GLB unique 요청, page error 0, cleanup 후 scene children 0 및 objectMap 0. |
| 주의 사항 | 앱 루트가 함께 뜨면 도슨트 drone asset이 측정에 섞일 수 있어, 최종 스크립트는 앱을 마운트하지 않고 측정 harness 모듈만 import한다. |
| 다음 작업 | TASK-029를 정리 단계로 넘긴다. 추가 최적화가 필요하면 프레임 타임/메모리 사용량 계측으로 확장한다. |

## 2026-06-22: TASK-029 GalleryScene 전환 asset cache 측정

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료된 부분 | `frontend/scripts/measure-gallery-scene-transitions.mjs`, `frontend/src/three/GallerySceneMeasureHarness.jsx`, `npm run measure:scene-transitions`를 추가했다. 실제 `GalleryScene` 컴포넌트 마운트 후 hall id를 변경해 특수관 진입/재진입의 asset cache와 cleanup 상태를 측정한다. |
| 실행 방법 | 프론트 dev server 실행 후 `npm.cmd run measure:scene-transitions -- --chrome="C:/Program Files/Google/Chrome/Application/chrome.exe"` |
| 검증 | `node --check scripts/measure-gallery-scene-transitions.mjs` 성공, `npm run build` 성공, 측정 스크립트 실행 성공. |
| 측정 결과 | Space cold resources 5 / warm 0, Greek cold resources 5 / warm 0. canvas 1개, webgl canvas 1개 유지. 네트워크 asset 요청 12개, unique URL 12개, page error 0. |
| 주의 사항 | 측정은 `GalleryScene` 직접 마운트 기반이라 실제 키보드 이동으로 포탈 충돌을 발생시키지는 않는다. 다만 scene cleanup, runtime import, 특수관 content 생성 경로는 실제 컴포넌트 경로를 통과한다. |
| 다음 작업 | 원격 방문자 다중 생성 시 `loadBlockyCharacter()` 캐시 효과와 per-user clone/animation mixer 비용을 측정한다. |

## 2026-06-22: TASK-029 runtime asset cache 측정

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료된 부분 | `frontend/scripts/measure-gallery-runtime-assets.mjs`와 `npm run measure:runtime-assets`를 추가했다. 우주관/역사관 런타임 생성 함수를 브라우저에서 cold/warm으로 실행해 실제 생성 경로의 asset 요청을 측정한다. |
| 실행 방법 | 프론트 dev server 실행 후 `npm.cmd run measure:runtime-assets -- --chrome="C:/Program Files/Google/Chrome/Application/chrome.exe"` |
| 검증 | `node --check scripts/measure-gallery-runtime-assets.mjs` 성공, `npm run build` 성공, 측정 스크립트 실행 성공. |
| 측정 결과 | Space models 9개, Greek models 5개 기준 cold runtime 7.04s, warm runtime 7.03s. warm-only resource timing 없음. 네트워크 asset 요청 12개, unique URL 12개. |
| 주의 사항 | runtime 시간은 기본 settle wait 7초를 포함한다. 로더 캐시의 핵심 확인 지표는 warm-only resource timing 없음과 warm 구간 신규 네트워크 요청 없음이다. |
| 다음 작업 | 포탈 기반 실제 진입/재진입 전체 플로우와 원격 방문자 다중 생성 시나리오 측정으로 확장한다. |

## 2026-06-22: TASK-029 asset cache 측정 스크립트

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료한 부분 | `frontend/scripts/measure-gallery-assets.mjs`와 `npm run measure:assets`를 추가했다. |
| 실행 방법 | 프론트 dev server 실행 후 `npm.cmd run measure:assets -- --chrome="C:/Program Files/Google/Chrome/Application/chrome.exe"` |
| 검증 | `node --check scripts/measure-gallery-assets.mjs` 성공, `npm run build` 성공, 측정 스크립트 실행 성공. |
| 측정 결과 | 10개 모델 기준 cold 1.58s, warm 0.04s. warm-only resource timing 없음. |
| 주의 사항 | PowerShell에서는 `npm.ps1` 실행 정책에 막힐 수 있어 Windows에서는 `npm.cmd` 사용이 안전하다. |
| 다음 작업 | 포탈 기반 실제 특수관 진입/재진입 시나리오와 원격 방문자 다중 생성 시나리오를 측정에 추가한다. |

## 2026-06-22: TASK-029 도슨트/방문자 로더 캐시 전환

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료한 부분 | `createDocent.js`와 `loadBlockyCharacter.js`도 공통 `assetLoader.js` 경로로 전환했다. 캐릭터 로더를 위해 `loadGltfAsset()`이 scene clone과 animations를 함께 반환한다. |
| 검증 | 직접 `GLTFLoader`/`DRACOLoader` 생성은 `assetLoader.js`에만 남았다. `node --check`, `npm run build`, Playwright Main Gallery 렌더 확인 성공. |
| 주의 사항 | Playwright 검증은 백엔드 미구동 상태였다. `/api`, `/ws` 프록시 오류는 있었지만 브라우저 page error는 없었다. |
| 다음 작업 | 특수관 반복 진입, 원격 방문자 다중 생성 시 로딩 시간과 네트워크 요청 수를 측정해 캐시 효과를 기록한다. |

## 2026-06-22: TASK-029 GLTF Asset Loader 캐싱

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료한 부분 | `frontend/src/three/assetLoader.js`를 추가해 GLTF/DRACO 로더와 URL별 Promise 캐시를 공통화했다. `createAstronaut.js`, `createGeminiSpacesuit.js`, `createSpaceShuttle.js`, `createGreekSculpture.js`가 `loadGltfScene(url)`을 사용한다. |
| 검증 | `node --check` 대상 파일 성공, `npm run build` 성공, Playwright에서 Main Gallery 렌더 및 백엔드 프록시 제외 브라우저 오류 없음 확인. |
| 주의 사항 | scene cleanup이 geometry/material을 dispose하므로 `loadGltfScene()`은 캐시 원본을 직접 반환하지 않고 `SkeletonUtils.clone()`과 geometry/material clone을 거친다. |
| 다음 작업 | 도슨트 드론과 원격 방문자 캐릭터 로더에도 같은 캐시를 적용할지 검토한다. 실제 특수관 재진입을 반복 측정해 캐시 효과를 기록한다. |

## 2026-06-22: TASK-029 2차 room InstancedMesh 최적화

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료한 부분 | `frontend/src/three/buildRoom.js`의 몰딩 8개와 바닥 격자 10개를 개별 Mesh 대신 치수별 `InstancedMesh`로 생성하도록 바꿨다. |
| 검증 | `node --check frontend/src/three/buildRoom.js` 성공, `npm run build` 성공, Playwright 스크린샷에서 Main Gallery 방 구조/몰딩/격자 정상 렌더 확인. |
| 주의 사항 | Playwright 확인 중 백엔드 미구동으로 `/api`와 `/ws` 프록시 오류가 났지만 이번 3D 렌더 검증과는 별개다. |
| 다음 작업 | GLB Asset Loader 캐싱 전략을 설계하고, 전역 geometry/material cache와 scene cleanup 정책을 함께 정리한다. |

## 2026-06-22: TASK-029 1차 room geometry 최적화

| 항목 | 내용 |
| --- | --- |
| 현재 진행 중인 작업 | TASK-029 3D 리소스 관리 및 렌더링 성능 최적화 |
| 완료한 부분 | `frontend/src/three/buildRoom.js`에서 방 테마 설정을 `ROOM_THEMES`로 분리하고, 박스 geometry 캐시와 역사관 기둥 geometry 공유를 적용했다. |
| 검증 | `node --check frontend/src/three/buildRoom.js` 성공, `npm run build` 성공. WebLLM 대형 chunk 경고는 기존 기술 부채로 유지된다. |
| 주의 사항 | `GalleryScene.jsx` cleanup이 scene 내 mesh geometry/material을 dispose하므로 이번 공유 geometry는 CPU 객체 재사용 효과가 중심이다. GPU 리소스 유지나 전역 asset cache는 별도 정책이 필요하다. |
| 다음 작업 | 몰딩/바닥 격자 `InstancedMesh` 적용 여부를 검토하고, 이후 GLB Asset Loader 캐싱 전략을 결정한다. |
