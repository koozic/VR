...
- 원격 유저가 정지한 뒤에도 걷기 모션이 계속 재생되거나 패킷 사이에서 걷기/대기가 반복 전환되는 문제를 수정했다.
- `createRemoteVisitor.js`에서 이동 상태가 실제로 변경될 때만 걷기/대기 애니메이션 전환을 실행하고 크로스페이드 시간을 150ms로 조정했다.
- `syncRemoteVisitors.js`에서 서버 원본 좌표 변화만 이동으로 판단하고 마지막 이동 후 120ms 동안 걷기 상태를 유지하도록 보완했다.
- `loadBlockyCharacter.js`에서 블록 캐릭터의 기본 정면 축을 `Math.PI`만큼 보정했다.
- `npm run build` 통과. 다중 브라우저에서 이동 후 정지 모션 수동 검증은 아직 필요하다.
...
> 2026-06-22 추가 상태 메모: `frontend/scripts/measure-remote-visitors.mjs`, `frontend/src/three/remoteVisitorMeasureHarness.js`, `npm run measure:remote-visitors`를 추가해 원격 방문자 18명 다중 생성/제거 시 캐릭터 GLB 캐시 효과를 측정했다. primary cold 10 resources / warm 0, secondary cold 8 resources / warm 0으로 18종 캐릭터가 한 번씩만 네트워크 요청되고 재생성은 캐시 clone/mixer 생성만 수행됨을 확인했다. 각 배치 cleanup 후 scene children 0, objectMap 0, page error 0.

> 2026-06-22 추가 상태 메모: `frontend/scripts/measure-gallery-scene-transitions.mjs`, `npm run measure:scene-transitions`, `frontend/src/three/GallerySceneMeasureHarness.jsx`를 추가해 실제 `GalleryScene` 마운트 상태에서 Main -> Space/Greek -> Main -> Space/Greek 재진입 흐름을 측정할 수 있게 했다. Space cold resources 5 / warm 0, Greek cold resources 5 / warm 0, canvas 1개 유지, page error 0으로 확인했다.

> 2026-06-22 추가 상태 메모: `frontend/scripts/measure-gallery-runtime-assets.mjs`와 `npm run measure:runtime-assets`를 추가해 실제 우주관/역사관 런타임 생성 경로에서 asset cache 효과를 측정할 수 있게 했다. Space 9개 모델, Greek 5개 모델 기준 cold runtime 7.04s, warm runtime 7.03s, warm-only resource timing 없음으로 확인했다. runtime 시간은 settle wait 7초를 포함하므로 네트워크 재요청 여부 확인 목적의 기준값이다.

> 2026-06-22 추가 상태 메모: `frontend/scripts/measure-gallery-assets.mjs`와 `npm run measure:assets`를 추가해 공통 GLTF 로더의 cold/warm 캐시 효과를 측정할 수 있게 했다. 10개 모델 기준 cold 1.58s, warm 0.04s, warm-only resource timing 없음으로 확인했다.

> 2026-06-22 추가 상태 메모: 도슨트 드론(`createDocent.js`)과 원격 방문자 캐릭터(`loadBlockyCharacter.js`)도 공통 `assetLoader.js` 경로로 전환했다. `loadGltfAsset()`을 추가해 애니메이션 클립이 필요한 캐릭터 로더도 캐시된 원본에서 안전한 클론과 animations를 함께 받을 수 있게 했다.

> 2026-06-22 추가 상태 메모: GLTF/DRACO 공통 로더 `frontend/src/three/assetLoader.js`를 추가하고 우주관 GLB 3개, 그리스 조각 GLB 5개 로딩 경로를 URL별 Promise 캐시로 연결했다. scene cleanup이 dispose를 수행하므로 캐시 원본은 직접 scene에 넣지 않고 안전한 클론을 반환한다. 남은 후보는 도슨트 드론/원격 방문자 캐릭터 로더 적용 범위 검토다.

> 2026-06-22 추가 상태 메모: 2차 패치로 `buildRoom.js`의 몰딩과 바닥 격자를 치수별 `InstancedMesh`로 묶었다. `node --check`, `npm run build`, Playwright 스크린샷 확인을 통과했다. GLB Asset Loader 캐싱 전략은 아직 남아 있다.

> 2026-06-22 상태 메모: codex가 1차 개선에 착수했다. `frontend/src/three/buildRoom.js`에서 방 테마 분기 정리, 박스 geometry 캐시, 역사관 기둥 geometry 공유를 적용했다. GLB Asset Loader와 몰딩/바닥 격자 InstancedMesh 적용 범위 결정은 남아 있다.
...
#### 2026-06-22 TASK-029 정리 및 미처리 작업

- 완료 요약: `buildRoom.js` geometry 재사용 및 trim/grid `InstancedMesh`, 공통 `assetLoader.js` 기반 GLTF/DRACO Promise cache, 우주관/역사관/도슨트/원격 방문자 GLB 로더 전환, asset/runtime/scene transition/remote visitor 측정 스크립트 추가를 완료했다.
- 검증 요약: `npm run build`, `node --check` 계열 검증, Playwright 측정 스크립트 실행을 통과했다. 주요 측정에서 warm 재진입/재생성 구간 asset resource timing은 0으로 확인했다.
- 미처리 작업: 실제 키보드/포인터락 이동으로 포탈 충돌을 발생시키는 end-to-end 사용자 플로우 측정은 아직 자동화하지 않았다.
- 미처리 작업: 프레임 타임, FPS, JS heap, GPU memory 등 렌더링 성능 지표는 아직 수집하지 않았다.
- 미처리 작업: 전역 asset cache의 eviction/lifetime 정책은 아직 없다. 현재 정책은 페이지 세션 동안 URL별 Promise cache를 유지하는 방식이다.
- 미처리 작업: 측정 스크립트 사용법은 `ai_command`에만 기록되어 있고 별도 사용자/개발자 문서로 정리하지 않았다.
- 미처리 작업: Chrome headless 중심 검증만 수행했다. Edge/모바일/저사양 환경 수동 검증은 남아 있다.
...
### TASK-037: Space Gallery 전시 데이터 갱신 및 시야 기반 UI 선택

| 항목 | 내용 |
| --- | --- |
| 작업 ID | TASK-037 |
| 작업명 | Space Gallery 전시 데이터 갱신 및 시야 기반 UI 선택 |
| 상태 | DONE |
| 현재 담당자 | codex |
| 작업 목적 | Space Gallery 이미지 전시물의 제목, 이미지, 설명문을 최신 콘텐츠에 맞게 정리하고, UI에 표시되는 전시물 선택이 카메라 시야 방향을 반영하도록 개선한다. |
| 수정 가능 파일 | `shared/gallery-seed.json`, `frontend/src/curator/exhibitConversationProfiles.js`, `frontend/src/three/createExhibitFrame.js`, `frontend/src/three/GalleryScene.jsx`, `frontend/src/three/distanceCheck.js`, `ai_command/*` |
| 수정 금지 파일 | 백엔드/AI 서버 비즈니스 로직, DB 스키마, 인증/환경 설정 파일 |
| 완료 조건 | Cosmos Odyssey와 Stellar Cradle의 제목, GIF URL, 설명문, 대화 프로필이 반영되고, 카메라가 바라보는 전시물만 포커스/HUD 후보가 되며, 프론트엔드 빌드가 통과한다. |
| 현재 진행 상황 | 완료. `shared/gallery-seed.json` version 5로 갱신, 두 우주 이미지 전시물 데이터 정리, Space Gallery 작품 발광/미세 움직임 유지, 카메라 forward 벡터 기반 시야각 필터 적용. |
| 참고 문서 | `WORK_LOG.md` 2026-06-16 TASK-037, `HANDOVER.md` 2026-06-16 TASK-037 |
| 인수인계 메모 | seed 변경은 로컬/운영 DB에 반영되려면 backend가 새 version 5를 읽어 재시드해야 한다. 실제 브라우저에서는 Space Gallery에서 작품 정면/측면 접근 시 HUD와 정보 패널 전환을 수동 확인하면 된다. |

### TASK-038: WebLLM 응답속도 개선
...
| 상태 | DONE |
| 현재 담당자 | codex |
| 작업 목적 | 브라우저 내 WebLLM 도슨트 답변의 첫 응답 지연과 생성 지연을 줄인다. |
| 수정 가능 파일 | `frontend/src/api/webLlmApi.js`, `frontend/src/pages/GalleryPage.jsx`, `frontend/src/components/DocentSpeechBubble.jsx` |
| 수정 금지 파일 | 백엔드 외부 AI API 동작, Gemini/API 키 환경 설정 |
| 완료 조건 | WebLLM 엔진을 첫 질문 전에 가능한 시점에 preload하고, 생성 토큰/대화 이력/프롬프트 길이를 줄이며, 프론트엔드 빌드가 통과한다. |
| 현재 진행 상황 | 완료. `prepareWebLlmModel()`로 페이지 진입 후 idle 시점에 WebLLM 엔진을 사전 준비하고, 생성은 `stream: true`와 `onToken`으로 중간 응답을 표시한다. 최근 대화 입력은 최대 6개, `max_tokens`는 160으로 축소됐다. 모델 준비 상태는 도슨트 말풍선에 preparing/ready/error로 표시된다. |
| 검증 | 2026-06-18 기준 `npm run build` 성공. Playwright + Chrome WebGPU 환경에서 이전 커밋(`7f1418e`)과 현재 커밋(`4622bf4`)을 비교했다. 이전 cold 요청은 약 46.24~48.22초였고 중간 표시가 없었다. 현재는 사전 준비 약 24.99~33.97초, warm 생성 14.67~19.48초, 첫 토큰 표시 6.39~8.97초를 확인했다. 실제 앱 UI에서도 약 27.51초 후 `AI 모델 준비 완료`가 표시됐다. |
| 참고 문서 | `WORK_LOG.md` 2026-06-16 WebLLM response latency improvement notes, `WORK_LOG.md` 2026-06-18 WebLLM 응답속도 개선 반영 및 성능 검증, `TECHNICAL_DEBT.md` 10번 항목 |
| 인수인계 메모 | 체감 속도는 스트리밍과 사전 로딩으로 개선됐다. 순수 warm 생성 시간은 이전 대비 큰 폭으로 줄었다고 보기는 어렵다. 최초 방문자는 여전히 모델 fetch/cache와 WebGPU 초기화 시간이 필요하며, 스트리밍 중간 텍스트는 최종 검증 전 화면에 표시될 수 있으므로 사실 검증 정책을 후속 점검한다. |

### TASK-039: WebLLM 최초 다운로드 UX 및 재시도 보강

| 항목 | 내용 |
| --- | --- |
| 작업 ID | TASK-039 |
| 작업명 | WebLLM 최초 다운로드 UX 및 재시도 보강 |
| 상태 | DONE |
| 현재 담당자 | codex |
| 작업 목적 | 최초 방문자가 WebLLM 모델 다운로드/캐시/초기화 상태를 이해하고, 실패 시 앱을 새로고침하지 않고 다시 시도할 수 있게 한다. |
| 수정 가능 파일 | `frontend/src/api/webLlmApi.js`, `frontend/src/pages/GalleryPage.jsx`, `frontend/src/components/DocentSpeechBubble.jsx`, `frontend/src/styles.css`, 필요 시 신규 WebLLM 상태 컴포넌트 |
| 수정 금지 파일 | 백엔드 외부 AI API 동작, Gemini/API 키 환경 설정 |
| 완료 조건 | 모델 준비 상태를 한국어 단계(`다운로드 중`, `캐시에서 불러오는 중`, `초기화 중`, `준비 완료`, `실패`)로 표시하고, 실패 상태에서 재시도 버튼으로 `prepareWebLlmModel()`을 다시 실행할 수 있다. 최초 다운로드 용량과 네트워크 필요성을 안내하며 `npm run build`가 통과한다. |
| 현재 진행 상황 | 완료. `frontend/src/api/webLlmApi.js`에서 MLC 원문 progress를 한국어 단계 메시지로 매핑하도록 변경했다. `DocentSpeechBubble.jsx`에 모델 준비 실패 전용 재시도 버튼을 추가했고, `GalleryPage.jsx`에서 재시도 시 `prepareWebLlmModel()`을 다시 실행한다. |
| 검증 | `npm run build` 성공. 기존 WebLLM 대형 chunk 경고는 유지된다. |
| 참고 문서 | `TECHNICAL_DEBT.md` 10번 항목, `WORK_LOG.md` 2026-06-18 WebLLM 응답속도 개선 반영 및 성능 검증 |
| 인수인계 메모 | 모델 준비 재시도는 도슨트 응답 재시도와 분리했다. 모델 로딩 실패 상태에서만 `모델 다시 준비` 버튼이 표시된다. |

### TASK-040: WebLLM 스트리밍 안전 표시 및 렌더링 최적화

| 항목 | 내용 |
| --- | --- |
| 작업 ID | TASK-040 |
| 작업명 | WebLLM 스트리밍 안전 표시 및 렌더링 최적화 |
| 상태 | DONE |
| 현재 담당자 | codex |
| 작업 목적 | 검증 전 생성 텍스트가 곧바로 사실처럼 노출되는 위험과 토큰 단위 상태 갱신으로 인한 렌더링 부담을 줄인다. |
| 수정 가능 파일 | `frontend/src/api/webLlmApi.js`, `frontend/src/pages/GalleryPage.jsx`, `frontend/src/components/DocentSpeechBubble.jsx`, `frontend/src/styles.css` |
| 수정 금지 파일 | 백엔드 외부 AI API 동작, Gemini/API 키 환경 설정 |
| 완료 조건 | 스트리밍 텍스트는 별도 `생성 중` 상태로 표시하거나 문장 단위/검증 단위로 노출한다. `onToken`에 100~200ms throttle 또는 `requestAnimationFrame` 기반 갱신 제한을 적용해 과도한 React 렌더링을 방지한다. 최종 응답만 세션 기록에 저장되며, fallback 검증 결과가 화면을 안정적으로 덮어쓴다. `npm run build`가 통과한다. |
| 현재 진행 상황 | 완료. 스트리밍 토큰은 최종 답변 본문에 바로 쓰지 않고 `생성 중인 초안` 영역에 분리 표시한다. `GalleryPage.jsx`에서 토큰 표시 갱신을 150ms 단위로 제한해 과도한 React 렌더링을 줄였다. 생성 완료 후 검증된 최종 메시지만 본문으로 반영하고 초안은 정리한다. |
| 검증 | `npm run build` 성공. 기존 WebLLM 대형 chunk 경고는 유지된다. |
| 참고 문서 | `TECHNICAL_DEBT.md` 10번 항목, `WORK_LOG.md` 2026-06-18 WebLLM 응답속도 개선 반영 및 성능 검증 |
| 인수인계 메모 | 현재 방식은 검증 전 스트리밍 텍스트를 별도 초안 영역으로 구분한다. 더 엄격한 사실 검증이 필요하면 문장 종결 단위 표시나 최종 검증 후 문장 단위 공개로 강화할 수 있다. |

### TASK-041: WebLLM 성능 기준 및 반복 검증 자동화

| 항목 | 내용 |
| --- | --- |
| 작업 ID | TASK-041 |
| 작업명 | WebLLM 성능 기준 및 반복 검증 자동화 |
| 상태 | TODO |
| 현재 담당자 | 미배정 |
| 작업 목적 | WebLLM 개선 여부를 감으로 판단하지 않고 첫 토큰 시간, 최종 생성 시간, 모델 준비 시간을 반복 측정해 기준값으로 관리한다. |
| 수정 가능 파일 | `frontend/scripts/`, `ai_command/VALIDATION_CHECKLIST.md` 또는 신규 검증 문서, 필요 시 `package.json` script |
| 수정 금지 파일 | 프로덕션 런타임 동작을 바꾸는 앱 소스 코드 |
| 완료 조건 | Playwright 또는 브라우저 수동 체크리스트로 cold/warm 모델 준비, 첫 토큰, 최종 응답 시간을 3~5회 반복 측정할 수 있다. 기준 예시는 warm 첫 토큰 8초 이하, warm 최종 응답 18초 이하이며, 측정 결과를 `WORK_LOG.md`에 기록한다. |
| 현재 진행 상황 | 일부 완료. `frontend/scripts/measure-webllm-performance.mjs`와 `npm run measure:webllm`을 추가해 Vite dev 서버에서 WebLLM 모델 준비 시간, 첫 토큰 시간, 최종 응답 시간, 토큰 업데이트 수를 반복 측정할 수 있게 했다. 수동 검증 기준과 기록 항목은 `docs/webllm-performance-checklist.md`에 정리했다. |
| 참고 문서 | `WORK_LOG.md` 2026-06-18 WebLLM 응답속도 개선 반영 및 성능 검증 |
| 인수인계 메모 | 측정 스크립트와 체크리스트 초안은 작성됐지만, 실제 3~5회 반복 측정 결과는 아직 `WORK_LOG.md`에 기록되지 않았다. 실제 모델 다운로드와 WebGPU 측정은 네트워크/브라우저/GPU 환경 영향을 받으므로 로컬 실브라우저에서 실행 결과를 별도로 남겨야 한다. |

### TASK-042: WebLLM 다중 브라우저 및 실패 환경 검증

| 항목 | 내용 |
| --- | --- |
| 작업 ID | TASK-042 |
| 작업명 | WebLLM 다중 브라우저 및 실패 환경 검증 |
| 상태 | TODO |
| 현재 담당자 | 미배정 |
| 작업 목적 | Chrome 단일 환경에서만 확인된 WebLLM 동작을 Edge, WebGPU 미지원, 다운로드 실패, 저사양/메모리 부족 환경에서도 검증한다. |
| 수정 가능 파일 | `ai_command/VALIDATION_CHECKLIST.md` 또는 신규 검증 문서, 필요 시 `frontend/src/api/webLlmApi.js` 오류 메시지 및 UI |
| 수정 금지 파일 | 검증 없이 WebLLM 모델 ID 변경, 백엔드 외부 AI API 동작 |
| 완료 조건 | Chrome/Edge/WebGPU 미지원/네트워크 실패/메모리 부족 또는 저사양 환경에서 모델 준비, 오류 메시지, 재시도, fallback 동작을 확인하고 결과를 `WORK_LOG.md`에 기록한다. |
| 현재 진행 상황 | 예정. 현재 검증은 Playwright + Chrome WebGPU 환경에서만 수행했다. TASK-041에서 반복 측정 스크립트와 체크리스트 초안은 마련했지만, Edge/Chrome 교차 검증, WebGPU 미지원, 네트워크 실패, 저사양/메모리 부족 검증은 아직 수행하지 않았다. |
| 참고 문서 | `TECHNICAL_DEBT.md` 10번 항목, `HANDOVER.md` 2026-06-18 WebLLM 응답속도 개선 반영 확인 |
| 인수인계 메모 | 지금 당장 완전 수행하기는 어렵다. Edge 설치/실기기/네트워크 차단/메모리 제약 환경이 필요하다. 다만 체크리스트 작성과 WebGPU 미지원 경로의 정적 검토는 바로 가능하다. |

### TASK-043: WebLLM 세션 히스토리 스코핑 및 전시물 오염 방지

| 항목 | 내용 |
| --- | --- |
| 작업 ID | TASK-043 |
| 작업명 | WebLLM 세션 히스토리 스코핑 및 전시물 오염 방지 |
| 상태 | TODO |
| 현재 담당자 | 미배정 |
| 작업 목적 | 세션 전체 대화 연속성은 유지하되, 전시물을 이동한 뒤 이전 전시물의 대화·관람 데이터가 현재 전시물 설명문에 섞이지 않도록 WebLLM 입력 컨텍스트를 분리한다. |
| 수정 가능 파일 | `frontend/src/pages/GalleryPage.jsx`, `frontend/src/api/webLlmApi.js`, `frontend/src/curator/`, 필요 시 `ai_command/ARCHITECTURE.md` |
| 수정 금지 파일 | 세션 히스토리 전체 삭제, 작품·전시관 이동 후 대화 기록 열람 기능 제거, 검증 없이 WebLLM 모델 ID 변경 |
| 완료 조건 | UI와 `sessionStorage`에는 전체 세션 히스토리가 유지된다. WebLLM 입력에는 현재 전시물 근거와 현재 전시물 관련 최근 대화를 우선 전달하고, 이전 전시물 대화는 사용자가 비교·회상·추천 의도를 명시한 경우에만 관련 과거 메시지 또는 방문 요약으로 제한 전달한다. 일반 설명 요청에서 이전 전시물 제목·작가·설명이 현재 전시물 답변에 섞이지 않는 것이 확인된다. |
| 현재 진행 상황 | 예정. 현재 구현은 `GalleryPage.jsx`에서 전체 `messages`를 WebLLM에 넘기고, `webLlmApi.js`에서 오류가 아닌 최근 메시지 6개를 context 구분 없이 사용하므로 전시물 이동 후 설명 오염 가능성이 있다. |
| 참고 문서 | `ARCHITECTURE.md` 세션 단위 대화형 큐레이터, `DECISIONS.md` ADR-009/ADR-010, `WORK_LOG.md` 2026-06-18 WebLLM 세션 히스토리 스코핑 검토 |
| 인수인계 메모 | 해결 방향은 히스토리 제거가 아니라 히스토리 스코핑이다. 현재 전시물 대화, 이전 전시물 대화, 세션 방문 요약을 분리하고, 질문에 `이전`, `아까`, `방금`, `전에`, `비교`, `비슷`, `추천`, `봤던`, `관람했던` 같은 회상·비교 의도가 있을 때만 이전 히스토리를 근거 컨텍스트로 포함한다. 프롬프트에는 “이전 대화는 관람 흐름 이해용이며, 현재 전시물 설명에는 현재 전시물 근거를 우선 사용한다”는 방어 문구를 추가한다. |
